...
const ( ResolutionDirect Resolution = "DIRECT"; ResolutionReconciled Resolution = "RECONCILED"; ResolutionUnknown Resolution = "UNKNOWN" )
...
type Engine struct {
	cfg *config.Config; risk *risk.Engine; pipeline Pipeline; registry *exchange.AdapterRegistry; oppCh chan ArbOpportunity
	mu sync.Mutex; dailyPnL float64
	inFlight map[string]bool; inFlightMu sync.Mutex
	accepting atomic.Bool; stopCh chan struct{}; stopOnce sync.Once; workers sync.WaitGroup
...
func NewEngine(cfg *config.Config, r *risk.Engine, p Pipeline, registry *exchange.AdapterRegistry) *Engine {
	e:=&Engine{cfg:cfg,risk:r,pipeline:p,registry:registry,oppCh:make(chan ArbOpportunity,256),inFlight:make(map[string]bool),stopCh:make(chan struct{})}; e.accepting.Store(true); return e
}
...
func (e *Engine) Start(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done(): log.Println("[Execution] Engine context cancelled."); return
		case <-e.stopCh: log.Println("[Execution] Engine intake stopped — draining in-flight executions."); return
		case opp:=<-e.oppCh:
			e.lifecycleMu.Lock(); if !e.accepting.Load() { e.lifecycleMu.Unlock(); continue }; e.workers.Add(1); e.lifecycleMu.Unlock()
...
func (e *Engine) Submit(opp ArbOpportunity) {
	e.lifecycleMu.Lock(); defer e.lifecycleMu.Unlock()
	if !e.accepting.Load() { log.Printf("[Execution] ⏭ Rejecting %s — engine is shutting down",opp.ID); return }
	select { case e.oppCh<-opp: default: log.Printf("[Execution] ⚠ Opportunity channel full — dropping %s",opp.ID) }
}

// Shutdown is ordered: close intake under the lifecycle lock, stop the
// dispatcher, then wait for every already-dispatched execution. The parent
// execution context must remain alive until this drain finishes so exchange
// reconciliation is not converted into an UNKNOWN state by SIGTERM itself.
func (e *Engine) Shutdown(ctx context.Context) error {
	e.lifecycleMu.Lock(); e.accepting.Store(false); e.stopOnce.Do(func(){close(e.stopCh)}); e.lifecycleMu.Unlock()
	done:=make(chan struct{}); go func(){e.workers.Wait(); close(done)}()
	select { case <-done: log.Println("[Execution] Graceful shutdown complete — no in-flight executions remain."); return nil; case <-ctx.Done(): return fmt.Errorf("execution shutdown deadline exceeded with in-flight work still active: %w",ctx.Err()) }
}
...
func (e *Engine) executeTwoLeg(ctx context.Context,opp ArbOpportunity){
	e.inFlightMu.Lock(); if e.inFlight[opp.Leg1.Symbol]{e.inFlightMu.Unlock();log.Printf("[Execution] ⏭ Skipping %s — %s already has an opportunity in flight",opp.ID,opp.Leg1.Symbol);return}; e.inFlight[opp.Leg1.Symbol]=true; e.inFlightMu.Unlock()
	...
	event.State="LEG1_PENDING"; leg1Result:=e.submitIOC(ctx,opp.Leg1,e.cfg.RiskParams.LegTimeout(opp.Leg1.Exchange)); event.Leg1Result=leg1Result
	...
	event.State="LEG1_FILLED"; elapsed:=time.Since(start); remainingBudget:=e.cfg.RiskParams.LegTimeout(opp.Leg1.Exchange)+e.cfg.RiskParams.LegTimeout(opp.Leg2.Exchange)+200*time.Millisecond-elapsed
	...
	if leg2Result.Filled&&leg2Result.FilledQty>=leg2Order.Quantity-1e-12{event.State="COMPLETED";event.Outcome="CLEAN";event.PnL=e.calculatePnL(leg1Result,leg2Result);event.TotalLatency=time.Since(start);e.mu.Lock();e.dailyPnL+=event.PnL;e.mu.Unlock();e.risk.RecordCleanExecution(event.PnL)}else{residual:=leg1Result.FilledQty-leg2Result.FilledQty;if residual<0{residual=0};if residual<=1e-12{event.State="COMPLETED";event.Outcome="CLEAN";event.PnL=e.calculatePnL(leg1Result,leg2Result);event.TotalLatency=time.Since(start)}else{e.reverseHedge(ctx,opp,leg1Result,residual,&event)}}
	e.emitEvent(event)
}
...
func(e *Engine)reverseHedge(ctx context.Context,opp ArbOpportunity,leg1Result FillResult,quantity float64,event *ExecutionEvent){if quantity<=1e-12{event.State="COMPLETED";event.Outcome="CLEAN";return};reverseSide:=SideSell;if opp.Leg1.Side==SideSell{reverseSide=SideBuy};reverseOrder:=Order{ID:fmt.Sprintf("%s-REVERSE",opp.ID),Exchange:opp.Leg1.Exchange,Symbol:opp.Leg1.Symbol,Side:reverseSide,Type:OrderMarket,Quantity:quantity};reverseResult:=e.submitIOC(ctx,reverseOrder,e.cfg.RiskParams.LegTimeout(reverseOrder.Exchange));if reverseResult.Resolution==ResolutionUnknown{event.State="UNKNOWN_ORDER_STATE";event.Outcome="KILL_SWITCH";e.risk.TripKillSwitch(fmt.Sprintf("UNKNOWN_ORDER_STATE: opp=%s leg=reverse order=%s exchange=%s",opp.ID,reverseResult.OrderID,reverseResult.Exchange));return};if reverseResult.Filled&&reverseResult.FilledQty>=quantity-1e-12{event.State="REVERSED";event.PnL=e.calculateReverseSlippage(leg1Result,reverseResult);event.Outcome="REVERSED";e.mu.Lock();e.dailyPnL+=event.PnL;e.mu.Unlock();e.risk.RecordReversal(event.PnL)}else{event.State="UNHEDGED_EXPOSURE";event.Outcome="KILL_SWITCH";e.risk.TripKillSwitch(fmt.Sprintf("UNHEDGED_EXPOSURE: opp=%s leg1_fill=%.4f",opp.ID,leg1Result.FilledQty))}}

func(e *Engine)submitIOC(ctx context.Context,order Order,timeout time.Duration)FillResult{timeoutCtx,cancel:=context.WithTimeout(ctx,timeout);defer cancel();resultCh:=make(chan FillResult,1);submitted:=time.Now();go func(){exOrder:=exchange.Order{ID:order.ID,Symbol:order.Symbol,Side:exchange.Side(order.Side),Type:exchange.OrderType(order.Type),Quantity:order.Quantity,LimitPrice:order.LimitPrice};exResult:=e.registry.SubmitIOC(timeoutCtx,order.Exchange,exOrder);result:=FillResult{OrderID:exResult.OrderID,Filled:exResult.Filled,FilledQty:exResult.FilledQty,AvgPrice:exResult.AvgPrice,Exchange:exResult.Exchange,Latency:time.Since(submitted),Resolution:ResolutionDirect,Error:exResult.Error};select{case resultCh<-result:case <-ctx.Done():}}();select{case result:=<-resultCh:return result;case <-timeoutCtx.Done():if ctx.Err()!=nil{return FillResult{OrderID:order.ID,Exchange:order.Exchange,Latency:time.Since(submitted),Resolution:ResolutionUnknown,Error:fmt.Errorf("context cancelled before order state resolved: %w",ctx.Err())}};return e.reconcileTimedOutOrder(ctx,order,submitted,timeout);case <-ctx.Done():return FillResult{OrderID:order.ID,Exchange:order.Exchange,Latency:time.Since(submitted),Resolution:ResolutionUnknown,Error:fmt.Errorf("context cancelled: %w",ctx.Err())}}
}
func(e *Engine)reconcileTimedOutOrder(parent context.Context,order Order,submitted time.Time,submitTimeout time.Duration)FillResult{reconcileCtx,cancel:=context.WithTimeout(parent,reconciliationTimeout);defer cancel();status:=e.registry.ReconcileOrder(reconcileCtx,order.Exchange,order.ID,order.Symbol);latency:=time.Since(submitted);switch status.State{case exchange.OrderStateFilled:return FillResult{OrderID:order.ID,Filled:true,FilledQty:status.FilledQty,AvgPrice:status.AvgPrice,Exchange:order.Exchange,Latency:latency,Resolution:ResolutionReconciled,Error:status.Error};case exchange.OrderStatePartiallyFilled:return FillResult{OrderID:order.ID,Filled:status.FilledQty>0,FilledQty:status.FilledQty,AvgPrice:status.AvgPrice,Exchange:order.Exchange,Latency:latency,Resolution:ResolutionReconciled,Error:status.Error};case exchange.OrderStateCancelled,exchange.OrderStateRejected,exchange.OrderStateNotFound:return FillResult{OrderID:order.ID,Filled:false,FilledQty:status.FilledQty,AvgPrice:status.AvgPrice,Exchange:order.Exchange,Latency:latency,Resolution:ResolutionReconciled,Error:status.Error};default:err:=status.Error;if err==nil{err=fmt.Errorf("exchange order state unresolved after %v reconciliation window (submit timeout %v)",reconciliationTimeout,submitTimeout)};return FillResult{OrderID:order.ID,Filled:status.FilledQty>0,FilledQty:status.FilledQty,AvgPrice:status.AvgPrice,Exchange:order.Exchange,Latency:latency,Resolution:ResolutionUnknown,Error:err}}}
